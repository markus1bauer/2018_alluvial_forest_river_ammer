# Show Figure graminoid's cover ratio ###
# Markus Bauer
...
(sites849318 <- select(sites849318, ID, plot, block, year, graminoidCovratio))
(sites0318 <- select(sites0318, ID, plot, block, year, graminoidCovratio))

#### Chosen model ###
m849318 <- lmer((graminoidCovratio) ~ year + (1|block/plot), sites849318, REML = F)
m0318 <- lmer((graminoidCovratio) ~ year + (1|block/plot), sites0318, REML = F)
...
###1984-1993-2018-----------------------------------------------------------------------------------------------------------------------
graph849318 <- ggplot(sites849318, aes(year, graminoidCovratio)) +
  geom_boxplot(color = "black") +
  annotate("text", x = 1, y = 1, label = "a", size = 3) +
  annotate("text", x = 2, y = 1, label = "b", size = 3) +
  annotate("text", x = 3, y = 1, label = "c", size = 3) + 
  labs(x = "", y = "Graminoid rel. abun.") +
  scale_y_continuous(limits = c(0,1), breaks = seq(0, 100, 0.2)) +
  scale_x_discrete(labels = c("1984","1993","2018")) +
  themeMB()

###2003-2018-------------------------------------------------------------------------------------------------------------------------
graph0318 <- ggplot(sites0318, aes(year, graminoidCovratio))+
  geom_boxplot(color = "black")+
  annotate("text", x = 1, y = 1, label = "a", size = 3)+
  annotate("text", x = 2, y = 1, label = "b", size = 3)+
  labs(x = "", y = "")+
...
ggsave("figure_5_graminoids_coverratio_(800dpi_8x5cm).tiff", 
