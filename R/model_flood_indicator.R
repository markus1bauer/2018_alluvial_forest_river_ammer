...
# Model for Ellenberg floodIndicator ####
# Markus Bauer
...
sites <- read_csv("data_processed_sites.csv", col_names = TRUE,
  ...
  select(id, treatment, floodRichness) %>%
  rename(value = floodRichness)
...
### Save ###
write.csv(
  tidytable, here("outputs", "statistics", "table_anova_flood_indicator.csv")
  )